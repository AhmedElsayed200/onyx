...
@contextmanager
def _mock_db_session() -> Iterator[MagicMock]:
    yield MagicMock()
...
def _assert_access_denied_response(
    ...
    with (
        patch(
            f"{_HANDLE_REGULAR_ANSWER}.get_session_with_current_tenant",
            side_effect=lambda: _mock_db_session(),
        ),
    ...
    mock_get_persona_by_id.assert_called_once()
...
def test_private_channel_non_ephemeral_generates_after_persona_access_check() -> None:
    ...
    assert result is False
    mock_get_persona_by_id.assert_called_once()
    assert mock_get_persona_by_id.call_args.kwargs["is_for_edit"] is False
...
def test_restricted_agent_successful_channel_answer_is_sent_only_to_sender() -> None:
    anonymous_user = MagicMock()
    client = MagicMock()
    ...
    with (
        ...
        patch(
            f"{_HANDLE_REGULAR_ANSWER}.get_persona_by_id",
            return_value=_make_slack_channel_config(is_public=False).persona,
        ),
        patch(f"{_HANDLE_REGULAR_ANSWER}.rate_limits", side_effect=_identity_decorator),
        patch(
            f"{_HANDLE_REGULAR_ANSWER}.retry_builder", side_effect=_identity_decorator
        ...
        result = handle_regular_answer(
            message_info=_make_slack_message_info(ChannelType.PRIVATE_CHANNEL),
            slack_channel_config=_make_slack_channel_config(is_public=False),
            receiver_ids=None,
            client=client,
            channel="C123",
            logger=_mock_logger(),
    ...
    assert mock_respond.call_args.kwargs["send_as_ephemeral"] is False
